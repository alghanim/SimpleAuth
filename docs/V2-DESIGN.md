...
4. **App self-management API** — `/api/app/*` + Basic/management-token auth + app-scoped `bootstrap`. ✅ **Done** (`requireApp` authenticates by HTTP Basic `app_id:app_secret` or a Bearer **app-management token** from `POST /api/app/token`; the app_id is derived from the credential and never from the path, so an app can only touch its own scope. Endpoints: `POST /api/app/bootstrap` (idempotent authz-as-code), `GET/PUT /api/app/authz`, `GET /api/app/settings`. Tests: `TestAppSelfService` incl. cross-app scoping. This is the DX payoff — a dev's whole integration is *get app_id/secret → bootstrap on deploy → point the SDK with `audience` → verify*.)
5. **App-local users** — `owner_app_id`, provisioning API, app-scoped login. ✅ **Done** (`User.OwnerAppID` marks an app-owned local identity; provisioned via `POST /api/app/users` — gated by `allow_local_users` — keyed by an `applocal:{app_id}` mapping so usernames are unique *per app*. `authenticateUser` checks app-local **first**, so an app-local user shadows a directory user of the same name at that app; app-local users only ever get `aud=owner-app` tokens, are **not** cross-app SSO-shared, and are **exempt** from `require_assignment`. Provisioning/list/delete/password under `/api/app/users`, scoped to the owning app. Tests: `TestAppLocalUsers`. Also hardened M3: a user is now assignable by **any** of their identity-mapping usernames.)
6. **SDKs + examples + docs** — all four SDKs to parity, examples reworked, README/docs guide (per the documentation standard). ✅ **Done** (all four SDKs — Go, JS/TS, Python, .NET — gained an app-management surface: `app_id`/`app_secret` construction + `appBootstrap`, `get/setAppAuthz`, `appSettings`, and app-local-user CRUD over HTTP Basic, reusing the existing `audience` verify option. New `app-integration` example per language showing bootstrap-on-deploy + audience-pinned verify. SDK READMEs + main README "Per-App Authorization (v2)" section + API.md "Apps (v2)" reference. Go SDK/examples build+vet clean; Python compiles + wire-tested; JS type-checks (modulo a pre-existing env-only typing artifact); .NET hand-verified — no toolchain in CI yet.)
7. **Admin UI** — apps management, per-app roles/assignments. ✅ **Done** (new "Apps" section in the embedded admin UI: list/register apps with the `app_secret` shown once, rotate secret, toggle `require_assignment`/`allow_local_users`, delete, and an authorization editor — roles (chips), user/group assignments (add/remove), and role→permission JSON — backed by `GET/PUT /api/admin/apps/{id}/authz`. Smoke-tested: UI served + apps API round-trips.)
