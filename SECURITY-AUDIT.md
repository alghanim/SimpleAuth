...
| H6 | `require_assignment` fails open when an app has no per-app authz | HIGH | OPEN | — (v2 branch, pre-release) |
...
| M14 | Deleting an app-local user leaves a dangling mapping (username unprovisionable) | MEDIUM | OPEN | — (v2 branch, pre-release) |
